...
export function DestinationChainSelector({
  ...
  const { destinationChain } = useWatch({ control });
      ...
      <div className="flex items-center justify-start h-12 p-2 bg-color-paper-darker rounded-xl">
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger className="p-0 [outline:none]">
            <div
              className="w-[200px] h-8 p-2.5 flex items-center justify-between rounded-xl text-color-text-paper bg-color-paper hover:bg-color-primary hover:text-color-button-text transition-all duration-[200ms] ease-in-out [outline:none]"
              onClick={() => setOpen(true)}
            >
              <span className="m-0 p-0 min-h-fit">{destinationChain}</span>
              ...
              {chainOptions.map((destination) => {
                return (
                  <button
                    ...
                    onClick={() => {
                      setOpen(false);
                      setValue(namePeriodScale, destination);
                    }}
                  >
                    {destination}
                  </button>
